...
`ifmodels/ifmodels.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `ifmodels.py` in directory called
`ifmodels`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import ifmodels as sb` in an
interactive Python session, the classes and functions defined inside of the
`ifmodels.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the ifmodels module, the main classes defined at the bottom of
...
Either way, you can create a `ifmodels/data` folder in which you can
...
(typically, something like our `ifmodels/tests/test_ifmodels.py`). Within each
...
have one such test in `ifmodels/tests/test_ifmodels.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/ifmodels`),
and type:
...
will see a message such as:
    ...
    ifmodels/tests/test_ifmodels.py .F...
    ...
    ifmodels/tests/test_ifmodels.py:49: AssertionError
...
To document `ifmodels` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/ifmodels/blob/master/doc/_static/RTD-advanced-conf.png)

 http://ifmodels.readthedocs.org/en/latest/
...
A `ifmodels/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/ifmodels) for the software. This
...
`ifmodels` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `ifmodels`, we use the
...
[here](https://github.com/uwescience/ifmodels/blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "ifmodels" repository.
...
`ifmodels/ifmodels.py` to be called `smallish/smallish.py`

	git mv ifmodels smallish
	git mv smallish/ifmodels.py smallish/smallish.py
	git mv smallish/tests/test_ifmodels.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `ifmodels` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `ifmodels` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `ifmodels` in them. Type
the following to see where all these files are:

	git grep ifmodels

You can replace `ifmodels` for `smallish` quickly with:

	git grep -l 'ifmodels' | xargs sed -i 's/ifmodels/smallish/g'
...
Edit all the mentions of `ifmodels` in the `ifmodels/__init__.py` file, and in
the `ifmodels/version.py` file as well.
